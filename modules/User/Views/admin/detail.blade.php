    ...
    <form action="{{url('admin/module/user/store/'.($row->id ?? -1))}}" method="post" class="needs-validation" novalidate>
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->getDisplayName() : __("Add new user")}}</h1>
...
@section ('script.body')
    ...
    <script>
        ...
        let mapLat = {{ !empty($row->candidate) ? ($row->candidate->map_lat ?? setting_item('default_location_lat', "51.505")) : setting_item('default_location_lat', "51.505") }};
        let mapLng = {{ !empty($row->candidate) ? ($row->candidate->map_lng ?? setting_item('default_location_lng', "-0.09")) : setting_item('default_location_lng', "-0.09") }};
