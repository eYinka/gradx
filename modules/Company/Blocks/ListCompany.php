        ...
        $this->setOptions([
            'settings' => [
                ...
                [
                    ...
                    'select2' => [
                        'ajax'  => [
                            'url'      => route('company.admin.category.getForSelect2'),
                            'dataType' => 'json'
                        ],
                        'width' => '100%',
                        'allowClear' => 'true',
                    ...
                    'pre_selected'=> route('company.admin.category.getForSelect2', ['pre_selected' => 1])
                ],
                [
                    'id'            => 'order',
                    'type'          => 'radios',
    ...
    public function query($model){
        ...
        if(!empty($ids) && count($ids) > 0){
            ...
            if (!empty($model['category_id'])) {
                ...
                $list_cat = CompanyCategory::query()->whereIn('id', $category_ids)->where("status","publish")->get();
                if(!empty($list_cat)){
                    $where_left_right = [];
                    $params = [];
                    foreach ($list_cat as $cat){
                        $where_left_right[] = " ( bc_company_categories._lft >= ? AND bc_company_categories._rgt <= ? ) ";
                        $params[] = $cat->_lft;
                        $params[] = $cat->_rgt;
                    }
                    $sql_where_join = " ( ".implode("OR" , $where_left_right)." )  ";
                    $model_company
                        ->join('bc_company_categories', function ($join) use($sql_where_join,$params) {
        ...
        if(!empty($ids) && count($ids) > 0){
            ...
            return $model_company->with(['location'])->limit(50)->orderByRaw(DB::raw("FIELD(id, '$imploded_strings')"))->get();
        }else{
            return $model_company->with(['location'])->limit($model['number'] ?? 5)->get();
        }
    }
}
